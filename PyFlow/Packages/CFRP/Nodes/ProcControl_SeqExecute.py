...
class SeqExecute(NodeBase):
    def __init__(self, name):
        super(SeqExecute, self).__init__(name)

        self.inExec = self.createInputPin('Click Start', 'ExecPin', None, self.compute)
        ...
        self.steps=[
            self.createOutputPin('Step{}'.format(i), 'ExecPin') for i in range(5)
        ]
    ...
    @staticmethod
    def description():
        return 'Proc Control - Seq Execution'

    def compute(self, *args, **kwargs):

        for step in self.steps:
            result=step.call(*args,**kwargs)
